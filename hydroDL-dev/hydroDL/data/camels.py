...
def readForcingGage(usgsId, varLst, *, dataset, nt):
    ...
    ind = np.argwhere(gageDict['id'] == usgsId)[0][0]
    huc = gageDict['huc'][ind]
    if dataset=='fused_prcp':
        tempS = 'fused'
        dataFile = os.path.join('/data/kas7897/dPLHBVrelease/hydroDL-dev/example/', dataset,
                                str(huc).zfill(2),
...
def readAttr(usgsIdLst, varLst):
    ...
    try:
        ...
        indGrid = np.full(len(usgsIdLst), np.nan).astype(int)
    try:
...
def getStatDic(attrLst = None, attrdata=None, seriesLst = None, seriesdata=None):
    ...
    if seriesLst is not None:
        for k in range(len(seriesLst)):
            var = seriesLst[k]
            if var in ['prcp', 'Precip', 'runoff', 'Runoff', 'Runofferror', 'prcp_daymet', 'prcp_maurer', 'prcp_nldas']:
...
def transNormbyDic(x, varLst, staDic, *, toNorm):
    ...
    for k in range(len(varLst)):
        ...
        if toNorm is True:
            if len(x.shape) == 3:
                if var in ['prcp', 'usgsFlow', 'Precip', 'runoff', 'Runoff', 'Runofferror', 'prcp_daymet', 'prcp_maurer', 'prcp_nldas']:
