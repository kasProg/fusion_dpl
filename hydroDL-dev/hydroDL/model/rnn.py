...
class MultiInv_HBVTDModel(torch.nn.Module):
    ...
    def __init__(self, *, ninv, nfea, nmul, hiddeninv, drinv=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False, multiforcing=False, prcp_datatypes=1):
        # LSTM Inv + HBV Forward
        ...
        if multiforcing is True:
            # ntp = nhbvpm + nroutpm + nwtspm + 2*prcp_datatypes #last weight is of pet
            ntp = nhbvpm + nroutpm + nwtspm + prcp_datatypes #last weight is of pet
            # ntp = nhbvpm + nroutpm + nwtspm + 3
        else:
            ntp = nhbvpm + nroutpm + nwtspm
        # ntp = nfea*nmul+nmul+2
        # ntp = nfea * nmul + 2
        self.lstminv = CudnnLstmModel(
            nx=ninv, ny=ntp, hiddenSize=hiddeninv, dr=drinv)
        # self.lstminv = CudnnLstmModel(
        #     nx=ninv, ny=ntp+3, hiddenSize=hiddeninv, dr=drinv)
        if ETMod is True:
    ...
    def forward(self, x, z, prcp_loss_factor, smooth_loss_factor, multiforcing, doDropMC=False):
    # def forward(self, x, z, prcp_loss_factor, multiforcing, doDropMC=False):
        Params0 = self.lstminv(z)
        # for param in self.lstminv.parameters():
        ...
        if multiforcing is True:
            # prcp_pet_wghts = Params0[:,:,-self.prcp_datatypes**2:] #for all forcings with loss factor
            # prcp_pet_wghts = Params0[:,:,-self.prcp_datatypes-1:] #for pet with weights
            # prcp_pet_wghts = Params0[:,:,-self.prcp_datatypes-2:] #for pet and temp with weights
            # prcp_pet_wghts = Params0[:,:,-2*(self.prcp_datatypes):] #for prcp and pet with loss factor
            prcp_wghts = Params0[:,:,-self.prcp_datatypes:]

            # prcp_pet_wghts = torch.sigmoid(prcp_pet_wghts)
            prcp_pet_wghts = torch.sigmoid(prcp_wghts)
            

            prcp_pet_wghts_denorm = torch.zeros_like(prcp_pet_wghts)
            if prcp_pet_wghts.shape[2] == 1:
                prcp_scale = [0.9, 1.1]
                for ip in range(prcp_pet_wghts.shape[2]):
                    prcp_pet_wghts_denorm[:,:,ip] = prcp_scale[0] + prcp_pet_wghts[:,:,ip]*(prcp_scale[1]-prcp_scale[0])
                pet_wghts_denorm = prcp_pet_wghts_denorm[:, :, -1]
                prcp_wghts_denorm = prcp_pet_wghts_denorm[:, :, -self.prcp_datatypes-1:-1]
            else:
                # pet_scale = [0.9, 1.1]
                prcp_wghts_denorm = prcp_pet_wghts[:, :, :self.prcp_datatypes]
                # pet_wghts_denorm = pet_scale[0] + prcp_pet_wghts[:, :, -1]*(pet_scale[1]-pet_scale[0])
                # temp_wghts_denorm = pet_scale[0] + prcp_pet_wghts[:, :, -2]*(pet_scale[1]-pet_scale[0])
                # prcp_pet_wghts_denorm[:,:,:self.prcp_datatypes] = prcp_wghts_denorm
                # prcp_pet_wghts_denorm[:,:,-1] = pet_wghts_denorm
                # prcp_pet_wghts_denorm[:,:,-2] = temp_wghts_denorm

                #for all forcings with loss factor
                # pet_wghts_denorm = prcp_pet_wghts[:,:,-self.prcp_datatypes:]
                # prcp_wghts_denorm = prcp_pet_wghts[:,:,:self.prcp_datatypes]
                # temp_wghts_denorm = prcp_pet_wghts[:,:, self.prcp_datatypes:self.prcp_datatypes*2]

            prcp_wghts_sum = torch.sum(prcp_wghts_denorm, dim=2)
            # pet_wghts_sum = torch.sum(pet_wghts_denorm, dim=2)
            # temp_wghts_sum = torch.sum(temp_wghts_denorm, dim=2)

            prcp_wavg = torch.zeros((ntstep, ngage), requires_grad=True, dtype=torch.float32).cuda()
            # pet_wavg = torch.zeros((ntstep, ngage), requires_grad=True, dtype=torch.float32).cuda()
            # temp_wavg = torch.zeros((ntstep, ngage), requires_grad=True, dtype=torch.float32).cuda()
            for para in range(prcp_wghts_denorm.shape[2]):
                prcp_wavg = prcp_wavg + prcp_wghts_denorm[:,:,para]*x[:,:,para]
                #for only PET with loss factor
                # pet_wavg = pet_wavg + pet_wghts_denorm[:,:,para]*x[:,:,para+self.prcp_datatypes+1]
                #for both temp and PET with loss factor
                # temp_wavg = temp_wavg + temp_wghts_denorm[:,:,para]*x[:,:,para+self.prcp_datatypes]
                # pet_wavg = pet_wavg + pet_wghts_denorm[:,:,para]*x[:,:,para+self.prcp_datatypes+self.prcp_datatypes]
            ...
            x_new[:,:,2] = x[:,:,-1]
            # x_new[:,:,1] = temp_wavg
            # x_new[:,:,1] = x[:,:,-2]*temp_wghts_denorm
            # x_new[:,:,2] = x[:,:,-1]*pet_wghts_denorm
            # x_new[:,:,2] = pet_wavg

            range_bound_loss_prcp = self.RangeBoundLoss([prcp_wghts_sum], factor=prcp_loss_factor)
            # range_bound_loss_temp = self.RangeBoundLoss_pet([temp_wghts_sum], factor=prcp_loss_factor)
            range_bound_loss_temp = 0
            # range_bound_loss_pet = self.RangeBoundLoss_pet([pet_wghts_sum], factor=prcp_loss_factor)
            range_bound_loss_pet = 0
            smooth_loss = self.SmoothLoss(prcp_wghts_denorm, factor=smooth_loss_factor)
        else:
            x_new = x
        ...
        out = self.HBV(x_new, parameters=hbvpara, staind=self.staind, tdlst=self.tdlst, mu=self.nmul, muwts=wts, rtwts=routpara,
                          bufftime=self.inittime, routOpt=self.routOpt, comprout=self.comprout, dydrop=self.dydrop)
        if multiforcing is True:
            # return out, range_bound_loss, prcp_wghts_denorm
            return out, range_bound_loss_prcp, range_bound_loss_pet, range_bound_loss_temp, prcp_wghts_denorm, smooth_loss
            # return out, range_bound_loss_prcp, range_bound_loss_pet, range_bound_loss_temp, prcp_wghts_denorm, smooth_loss, slope_grad_daymet, slope_grad_maurer, slope_grad_nldas
            # return out, range_bound_loss_prcp, prcp_pet_wghts_denorm, smooth_loss
        else:
            return out


class MultiInv_HBVTDModel_1(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    def __init__(self, *, ninv, nfea, nmul, hiddeninv, drinv=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False, multiforcing=False, prcp_datatypes=1):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_1, self).__init__()
        self.ninv = ninv
        self.nfea = nfea
        self.hiddeninv = hiddeninv
        self.nmul = nmul
        ...
        # ub_pet = [1.1]
        self.RangeBoundLoss = RangeBoundLoss(lb=lb_prcp, ub=ub_prcp)
        # self.RangeBoundLoss_temp = RangeBoundLoss(lb=lb_prcp, ub=ub_prcp)
        # self.RangeBoundLoss_pet = RangeBoundLoss(lb=lb_prcp, ub=ub_prcp)
        self.SmoothLoss = SmoothLossFunction()
    ...
    def forward(self, x, z, prcp_loss_factor, smooth_loss_factor, multiforcing, doDropMC=False):
        ...
        # Params0.requires_grad = False
        # slope_grad_daymet = autograd.grad(outputs=Params0[:, :, -3], inputs=z, grad_outputs=torch.ones_like(Params0[:, :, -3]), retain_graph=True)[0]
        # slope_grad_maurer = autograd.grad(outputs=Params0[:, :, -2], inputs=z, grad_outputs=torch.ones_like(Params0[:, :, -2]), retain_graph=True)[0]
        # slope_grad_nldas = autograd.grad(outputs=Params0[:, :, -1], inputs=z, grad_outputs=torch.ones_like(Params0[:, :, -1]), retain_graph=True)[0]
        ...
        if multiforcing is True:
            ...
            # the gradients of all inputs (not only slope)
            # slope_grad_daymet = autograd.grad(outputs=prcp_pet_wghts[:, :, 0], inputs=z, grad_outputs=torch.ones_like(prcp_pet_wghts[:, :, 0]), retain_graph=True)[0]
            # slope_grad_maurer = autograd.grad(outputs=prcp_pet_wghts[:, :, 1], inputs=z, grad_outputs=torch.ones_like(prcp_pet_wghts[:, :, 1]), retain_graph=True)[0]
            # slope_grad_nldas = autograd.grad(outputs=prcp_pet_wghts[:, :, 2], inputs=z, grad_outputs=torch.ones_like(prcp_pet_wghts[:, :, 2]), retain_graph=True)[0]

            prcp_pet_wghts_denorm = torch.zeros_like(prcp_pet_wghts)
            if prcp_pet_wghts.shape[2] == 1:
                prcp_scale = [0.9, 1.1]
                for ip in range(prcp_pet_wghts.shape[2]):
                    prcp_pet_wghts_denorm[:, :, ip] = prcp_scale[0] + prcp_pet_wghts[:, :, ip] * (
                                prcp_scale[1] - prcp_scale[0])
                pet_wghts_denorm = prcp_pet_wghts_denorm[:, :, -1]
                prcp_wghts_denorm = prcp_pet_wghts_denorm[:, :, -self.prcp_datatypes - 1:-1]
            else:
                # pet_scale = [0.9, 1.1]
        ...
        if self.nwtspm == 0:  # simple average for multiple components
            ...
            wtspara = Params0[-1, :, -self.nwtspm:]
            wts = F.softmax(wtspara, dim=-1)  # softmax to make sure sum to 1
        out = self.HBV(x_new, parameters=hbvpara, staind=self.staind, tdlst=self.tdlst, mu=self.nmul, muwts=wts,
                       rtwts=routpara,
                       bufftime=self.inittime, routOpt=self.routOpt, comprout=self.comprout, dydrop=self.dydrop)
        if multiforcing is True:
            # return out, range_bound_loss, prcp_wghts_denorm
