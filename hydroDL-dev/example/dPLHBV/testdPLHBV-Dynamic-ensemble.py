...
rootOut = os.path.join(os.path.sep, 'data', 'kas7897', 'dPLHBVrelease', 'output')

predtestALL = np.full([len(gageid), len(TtestLst), 5, ensembles], np.nan)
gpuids = json.loads(sys.argv[2])
# attribute_list = json.loads(sys.argv[1])
# alpha_list = json.loads(sys.argv[1])
ensemble_list = json.loads(sys.argv[1])
# alpha_list[0] = 0.0
for ensemble in range(ensembles):

    ## GPU setting
    testgpuid = gpuids[ensemble]
    ...
    if forType == ['daymet', 'maurer_extended', 'nldas_extended']:
        if ensemble>=10:
            testsave_path = 'ensemble' + str(ensemble-9)+'_alpha' +'/CAMELSDemo/dPLHBV/' + puN + '/TDTestforc/' + 'TD' + "_".join(tdRepS) + '/all_extended_withloss'+str(prcp_loss_factor)+'/BuffOpt' + str(buffOptOri) + '/RMSE_para'+str(alpha) + '/' + str(testseed)
        else:
            testsave_path = 'ensemble' + str(ensemble+1)+'/CAMELSDemo/dPLHBV/' + puN + '/TDTestforc/' + 'TD' + "_".join(tdRepS) + '/all_extended_withloss'+str(prcp_loss_factor)+'/BuffOpt' + str(buffOptOri) + '/RMSE_para'+str(alpha) + '/' + str(testseed)
    else:
        testsave_path = 'ensemble' + str(ensemble+1)+'_alpha' +'/CAMELSDemo/dPLHBV/' + puN + '/TDTestforc/' + 'TD'+"_".join(tdRepS)+'/' + forType + \
    ...
    for ifold in range(1, len(tarIDLst)+1):
        ...
        if ensemble>=10:
            ...
            attrnewLst = ensemble_list[ensemble]
        ...
        predtestALL[nstart:nstart+nbasin, :, :, ensemble] = dataPred[:, TestBuff-runBUFF:, :]
    ...
    evaDict = [stat.statError(predtestALL[:,:,0, ensemble], obstestALL.squeeze())]  # Q0: the streamflow
    ...
    np.save(predFile, predtestALL[:,:,:,ensemble])
