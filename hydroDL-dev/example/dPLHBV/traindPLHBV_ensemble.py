...
prcp_loss_factor = 60 #used only when multiforcing is True; else does not matter
...
ensembleno = sys.argv[3]
rootOut = os.path.join(os.path.sep, 'data', 'kas7897', 'dPLHBVrelease', 'output', ensembleno)  # Model output root directory
...
# attrnewLst = json.loads(sys.argv[1])
...
if forType==['daymet', 'maurer_extended', 'nldas_extended']:
    ...
    optData = default.update(optData, tRange=TtrainLoad, varT=varFInv, varC=attrnewLst, subset=TrainLS, forType = 'daymet')
    ...
    # obsUN_daymet = dfTrain_daymet.getDataObs(doNorm=False, rmNan=False, basinnorm=False)
    obsUN = dfTrain_daymet.getDataObs(doNorm=False, rmNan=False, basinnorm=False)
    ...
    # obsUN_maurer = dfTrain_maurer.getDataObs(doNorm=False, rmNan=False, basinnorm=False)
    ...
    # obsUN_nldas = dfTrain_nldas.getDataObs(doNorm=False, rmNan=False, basinnorm=False)
    ...
    #     optData = default.update(optData, tRange=TtrainLoad, varT=varFInv, varC=attrnewLst, subset=TrainLS, forType=forType[i])
    #     dfTrain_ = camels.DataframeCamels(tRange=TtrainLoad, subset=TrainLS, forType=forType[i])
    #     forcUN_ = dfTrain_.getDataTs(varLst=varF, doNorm=False, rmNan=False)
    #     obsUN_ = dfTrain_.getDataObs(doNorm=False, rmNan=False, basinnorm=False)
    #
    #     forcUN[:,:,i] = forcUN_[:,:,0]
    #     dfInv_ = camels.DataframeCamels(tRange=TinvLoad, subset=TrainLS, forType=forType[i])
    #     forcInvUN_ = dfInv_.getDataTs(varLst=varFInv, doNorm=False, rmNan=False)
    #     attrsUN_ = dfInv_.getDataConst(varLst=attrnewLst, doNorm=False, rmNan=False)
    #     forcInvUN[:, :, i] = forcInvUN_[:, :, 0]
    # forcUN = np.array(forcUN)
...
alpha = float(sys.argv[1]) # a weight for RMSE loss to balance low and peak flow
...
exp_name = 'CAMELSDemo'
if forType==['daymet', 'maurer_extended', 'nldas_extended']:
    exp_disp = 'dPLHBV/' + puN + TDN + 'all_extended_withloss' + str(prcp_loss_factor) + '/BuffOpt'+str(buffOpt)+'/RMSE_para'+str(alpha)+'/' + str(randomseed) + \
           '/Fold' + str(testfoldInd)
elif forType==['daymet', 'maurer', 'nldas']:
    exp_disp = 'dPLHBV/' + puN + TDN + 'all_withloss' + str(prcp_loss_factor) + '/BuffOpt'+str(buffOpt)+'/RMSE_para'+str(alpha)+'/' + str(randomseed) + \
           '/Fold' + str(testfoldInd)
else:
...
if TDOpt is False:
    ...
    model = rnn.MultiInv_HBVModel(ninv=Ninv, nfea=Nfea, nmul=Nmul, hiddeninv=HIDDENSIZE, inittime=BUFFTIME,
                                      routOpt = routing, comprout=comprout, compwts=compwts, pcorr=pcorr, multiforcing=multiforcing)
    ...
    model = rnn.MultiInv_HBVTDModel(ninv=Ninv, nfea=Nfea, nmul=Nmul, hiddeninv=HIDDENSIZE, inittime=BUFFTIME,
                                  ...
                                  dydrop=dydrop, ETMod=ETMod, multiforcing=multiforcing)
    # dict only for logging
