...
exp_name = 'CAMELSDemo'
if forType==['daymet', 'maurer_extended', 'nldas_extended']:
    exp_disp = 'LSTM-dPLHBV/' + puN + TDN + 'allprcp_5withloss' + str(prcp_loss_factor) + 'smooth' + str(smooth_loss_factor) + '/BuffOpt'+str(buffOpt)+'/RMSE_para'+str(alpha)+'/' + str(randomseed) + \
...
Ninv_hbv = zTrain_hbv.shape[-1] + attrs.shape[-1]

if TDOpt is False:
    # model with all static parameters
    model_hbv = rnn.MultiInv_HBVModel(ninv=Ninv_hbv, nfea=Nfea, nmul=Nmul, hiddeninv=HIDDENSIZE, inittime=BUFFTIME,
                                      routOpt = routing, comprout=comprout, compwts=compwts, pcorr=pcorr, multiforcing=False, prcp_datatypes=1)
    ...
    model_hbv = rnn.MultiInv_HBVTDModel(ninv=Ninv_hbv, nfea=Nfea, nmul=Nmul, hiddeninv=HIDDENSIZE, inittime=BUFFTIME,
                                  routOpt=routing, comprout=comprout, compwts=compwts, staind=staind, tdlst=tdRep,
                                  dydrop=dydrop, ETMod=ETMod, multiforcing=False, prcp_datatypes=1)
    model = rnn.prcp_weights(ninv=Ninv, hiddeninv=HIDDENSIZE, prcp_datatypes=len(forType))
...
testout = "/data/kas7897/dPLHBVrelease/output/CAMELSDemo/dPLHBV/ALL" \
          "/TDTestforc/TD1_13/nldas/BuffOpt0/RMSE_para0.25/111111/Fold1" \
