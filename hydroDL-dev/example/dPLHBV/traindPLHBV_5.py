...
if type(forType) == list:
    ...
    # forcUN = np.empty([671,5478,1+len(forType)])
    # forcInvUN = np.empty([671,5478,1+len(forType)])
    #
...
exp_name = 'CAMELSDemo'
if forType==['daymet', 'maurer_extended', 'nldas_extended']:
    exp_disp = 'dPLHBV/' + puN + TDN + 'all_extended_withloss' + str(prcp_loss_factor) + 'smooth' + str(smooth_loss_factor) + '/BuffOpt'+str(buffOpt)+'/RMSE_para'+str(alpha)+'/' + str(randomseed) + \
