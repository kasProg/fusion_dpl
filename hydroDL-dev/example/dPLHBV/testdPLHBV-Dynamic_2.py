...
## Testing parameters
Ttest = [19801001, 20101001]  # testing period
# Ttest = [19891001, 19991001]  # PUB/PUR period
TtestLst = utils.time.tRange2Array(Ttest)
TtestLoad = [19801001, 20101001]  # could potentially use this to load more forcings before testing period as warm-up
...
if forType == ['daymet', 'maurer_extended', 'nldas_extended']:
    testsave_path = 'CAMELSDemo/dPLHBV/' + puN + '/TDTestforc/' + 'TD' + "_".join(tdRepS) + '/allprcp_withloss'+str(prcp_loss_factor)+'smooth'+str(smooth_loss_factor)+'/BuffOpt' + str(buffOptOri) + '/RMSE_para'+ str(alpha) + '/' + str(testseed)
...
for ifold in range(1, len(tarIDLst)+1):
    ...
    if multiforcing==True:
        seriesvarLst=['prcp_daymet', 'prcp_maurer', 'prcp_nldas', 'tmax', 'pet']
    else:
