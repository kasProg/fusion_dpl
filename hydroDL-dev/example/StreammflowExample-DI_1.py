...
if 2 in Action:
    ...
    predLst = list()
    # with open("/data/kas7897/dPLHBVrelease/hydroDL-dev/camels_cali.txt") as f:
    #     # optData['subset'] = f.read().splitlines()
    #     subset = [int(x) for x in f.read().split()]
    subset = 'All'
    for out in outLst:
        if interfaceOpt == 1:  # use the more interpretable version interface
            # load testing data
            ...
            optData = mDict['data']
            df = camels.DataframeCamels(
                subset=subset, tRange=tRange, forType=forType)
            x = df.getDataTs(
                varLst=optData['varT'],
                doNorm=False,
                rmNan=False)
    ...
    print("without-DI: NSE,KGE,lowRMSE, highRMSE, mean lowRMSE, and highRMSE of all basins: ", np.nanmedian(dataBox[0][0]),
